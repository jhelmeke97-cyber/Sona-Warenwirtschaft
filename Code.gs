...
function onOpen() {
  ...
  ui.createMenu('Warenwirtschaft (' + CONFIG.LOCATION_NAME + ')')
    ...
    .addItem('11. 📱 Tablet-Schnelleingabe: Schwund & Bruch erfassen', 'showTabletLossEntryDialog')
    .addItem('12. 🗑️ Schwund- & Bruchliste (SCHWUND_NEU) öffnen', 'openSchwundNeuView')
    .addItem('13. 🔄 Schwundliste mit aktuellen Einkaufspreisen synchronisieren', 'syncSchwundNeuPrices')
    .addItem('14. 📊 Wöchentlichen Schwund-Bericht (Sona Hub) anzeigen', 'showWeeklyLossReport')
    .addItem('15. 🌐 SCHWUND_NEU in Ziel-Tabelle bereitstellen', 'deploySchwundNeuToTargetSpreadsheetPrompt')
    .addSeparator()
    .addItem('16. 🔍 Plausibilitäts-Audit (Health-Check) ausführen', 'runManualHealthAudit')
    .addItem('17. 📁 Google Drive Rechnungsordner Verbindung testen', 'checkDriveFolderConnection')
    .addItem('18. ⏱️ Automatisierung (Täglich 00:00 + Freitags) aktivieren', 'setupAutomatedTriggers')
    .addItem('19. ✉️ Freitags-Wochenbericht per Email testen', 'testSendWeeklyReport')
    .addSeparator()
    .addItem('20. Nächste freie Artikel-ID generieren', 'promptNextArticleId')
    .addItem('21. Preisabweichungen prüfen', 'checkPriceAnomalies')
    .addToUi();
}
...
function onEdit(e) {
  try {
    ...
    if (sheetName === (CONFIG.NAME_SCHWUND_NEU || 'SCHWUND_NEU') || sheetName === 'SCHWUND_BRUCH') {
...
function initializeEntireSystemAndData() {
  ...
  try {
    ...
    ui.alert(
      ...
      '5. SCHWUND_NEU: Tablet-optimierte Schwunderfassung mit Live-Preisen aus Master-Zutaten\n' +
...
function cleanupOldSheets(ss) {
  const namesToKeep = [CONFIG.NAME_DASHBOARD, CONFIG.NAME_RECHNUNGEN, CONFIG.NAME_ARTIKEL, CONFIG.NAME_MASTER_ZUTATEN, CONFIG.NAME_WARENGRUPPEN, CONFIG.NAME_PRUEFUNG, CONFIG.NAME_SCHWUND_NEU, CONFIG.NAME_SCHWUND_BRUCH];
...
/**
 * ============================================================================
 * 1.8 SCHWUND & BRUCH 2.0 (TABLET-SCHNELLEINGABE, LIVE-PREISE AUS MASTER_ZUTATEN & SONA HUB ANBINDUNG)
 * ============================================================================
 */
const LOSS_STATIONS = [
  '🍣 Sushi-Bar',
...
function setupSchwundNeuSheet(ss) {
  if (!ss) ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(CONFIG.NAME_SCHWUND_NEU || 'SCHWUND_NEU');
  if (!sheet) sheet = ss.insertSheet(CONFIG.NAME_SCHWUND_NEU || 'SCHWUND_NEU');
  else sheet.clear();
  ...
  refreshSchwundNeuMasterDropdown(ss, sheet);
...
function refreshSchwundNeuMasterDropdown(ss, sheet) {
  if (!ss) ss = SpreadsheetApp.getActiveSpreadsheet();
  if (!sheet) sheet = ss.getSheetByName(CONFIG.NAME_SCHWUND_NEU || 'SCHWUND_NEU');
  const mzSheet = ss.getSheetByName(CONFIG.NAME_MASTER_ZUTATEN);
  if (!sheet || !mzSheet || mzSheet.getLastRow() < 2) return;
...
function openSchwundNeuView() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(CONFIG.NAME_SCHWUND_NEU || 'SCHWUND_NEU');
  if (!sheet) {
    setupSchwundNeuSheet(ss);
    sheet = ss.getSheetByName(CONFIG.NAME_SCHWUND_NEU || 'SCHWUND_NEU');
  }
  if (sheet) ss.setActiveSheet(sheet);
}

function syncSchwundNeuPrices(ss) {
  if (!ss) ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(CONFIG.NAME_SCHWUND_NEU || 'SCHWUND_NEU');
  const mzSheet = ss.getSheetByName(CONFIG.NAME_MASTER_ZUTATEN);
  const ui = SpreadsheetApp.getUi();
  if (!sheet || sheet.getLastRow() < 4 || !mzSheet || mzSheet.getLastRow() < 2) {
  ...
  if (ui) {
    ui.alert(
      ...
      `Erfolgreich ${updatedCount} Einträge mit den tagesaktuellen Einkaufspreisen aus MASTER_ZUTATEN aktualisiert.\n\n` +
...
function handleSchwundNeuEdit(sheet, row, col) {
  ...
  // Wenn Datum geändert wird (Spalte 2) -> ID, KW & Monat setzen
  if (col === 2) {
  ...
  // Wenn Zutat gewählt wird (Spalte 6) -> Preis & Einheit aus MASTER_ZUTATEN ziehen
  if (col === 6 || col === 7 || col === 8) {
    ...
    if (zutat) {
      ...
      if (priceInfo && priceInfo.price > 0) {
        const norm = normalizeLossQuantity(rawQty, rawUnit, zutat, priceInfo.unit || 'kg');
        sheet.getRange(row, 9).setValue(norm.baseQty);
        sheet.getRange(row, 10).setValue(norm.baseUnit);
        sheet.getRange(row, 11).setValue(priceInfo.price);
        sheet.getRange(row, 12).setValue(Math.round(norm.baseQty * priceInfo.price * 100) / 100);
...
/**
 * Holt alle verfügbaren Master-Zutaten mit Preisen für das Tablet-Interface
 */
function getMasterIngredientsForTablet() {
  ...
  const mzSheet = ss.getSheetByName(CONFIG.NAME_MASTER_ZUTATEN);
  if (!mzSheet || mzSheet.getLastRow() < 2) return [];
  ...
  data.forEach(r => {
    ...
    const unit = String(r[5] || 'kg').trim();
    const price = parseFloat(r[6]) || 0;
    if (name && !name.startsWith('Unbekannt')) {
      list.push({ name: name, unit: unit, price: price, kat: kat });
    }
  });
...
/**
 * Server-Funktion: Speichert einen Tablet-Schwundeintrag direkt in SCHWUND_NEU
 */
function saveTabletLossEntry(formData) {
  try {
    ...
    const priceInfo = getCalculationPriceForIngredient(zutat, null, ss);
    const calcPrice = (priceInfo && priceInfo.price > 0) ? priceInfo.price : 0;
    const masterUnit = (priceInfo && priceInfo.unit) ? priceInfo.unit : 'kg';
    const category = (formData.isNewCategory || 'Food');
...
/**
 * Zeigt einen wöchentlichen Controlling-Bericht über Schwund & Bruch
 */
function showWeeklyLossReport() {
  ...
  let msg = `📊 WÖCHENTLICHER SCHWUND- & BRUCHBERICHT (${CONFIG.LOCATION_NAME})\n\n`;
...
/**
 * Richtet das SCHWUND_NEU Tabellenblatt direkt in der verlinkten Ziel-Tabelle ein
 */
function deploySchwundNeuToTargetSpreadsheetPrompt() {
  const ui = SpreadsheetApp.getUi();
  ...
  try {
    ...
    setupSchwundNeuSheet(targetSs);
    ui.alert(
      'Erfolgreich bereitgestellt!',
      `Das Tabellenblatt "SCHWUND_NEU" wurde erfolgreich in der verknüpften Restaurant-Tabelle eingerichtet:\n\n` +
      `Tabelle: ${targetSs.getName()}\n` +
      `URL: https://docs.google.com/spreadsheets/d/${targetId}`,
      ui.ButtonSet.OK
    );
  } catch (err) {
    ui.alert('Fehler bei Bereitstellung', 'Konnte nicht auf die Ziel-Tabelle zugreifen:\n' + err.toString(), ui.ButtonSet.OK);
  }
}
